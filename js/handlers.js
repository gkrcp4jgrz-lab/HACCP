...
window.deleteEquipment = function(id) { if (!isManager() && !isSuperAdmin()) { showToast('Accès refusé','error'); return; } return deleteEquipment(id); };
window.deleteProduct = function(id) { if (!isManager() && !isSuperAdmin()) { showToast('Accès refusé','error'); return; } return deleteProduct(id); };
window.deleteSupplier = function(id) { if (!isManager() && !isSuperAdmin()) { showToast('Accès refusé','error'); return; } return deleteSupplier(id); };
window.toggleModule = function(k,v) { if (!isManager() && !isSuperAdmin()) { showToast('Accès refusé','error'); return; } return toggleModule(k,v); };
