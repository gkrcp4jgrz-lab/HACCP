...
function renderOrdersActive() {
  ...
  // Form
  h += '<div class="card"><div class="card-header">➕ Nouvelle commande</div><div class="card-body"><form onsubmit="handleOrder(event)">';
  ...
  h += '<button type="submit" class="btn btn-primary">✓ Ajouter à la commande</button></form></div></div>';
  ...
  if (toOrder.length > 0) {
    ...
    h += '<div class="card" style="border-left:4px solid var(--warning)"><div class="card-header">🛒 À commander <span class="badge badge-yellow" style="margin-left:auto">' + toOrder.length + '</span></div><div class="card-body">';
    ...
    Object.keys(bySupplier).forEach(function(supplier) {
      ...
      h += '<h4 style="margin:0;font-size:15px;font-weight:700">🏭 ' + esc(supplier) + ' <span class="badge badge-blue">' + items.length + ' article' + (items.length > 1 ? 's' : '') + '</span></h4>';
      h += '<button class="btn btn-primary btn-sm" onclick="markSupplierOrdered(\'' + esc(supplier).replace(/'/g,'\\\'') + '\')">📞 Tout commandé</button>';
      h += '</div>';
      ...
      items.forEach(function(o) {
        ...
        h += '<div><strong>' + esc(o.product_name) + '</strong><span style="color:var(--gray);font-size:13px;margin-left:8px">' + (o.quantity||1) + ' ' + esc(o.unit||'unité') + '</span>';
  ...
  if (ordered.length > 0) {
    ...
    h += '<div class="card" style="border-left:4px solid var(--primary)"><div class="card-header">📦 En attente de livraison <span class="badge badge-blue" style="margin-left:auto">' + ordered.length + '</span></div><div class="card-body">';
    
    Object.keys(bySupplier2).forEach(function(supplier) {
      var items = bySupplier2[supplier];
      h += '<div style="margin-bottom:16px">';
      h += '<h4 style="margin:0 0 8px;font-size:14px;font-weight:700;color:var(--primary)">🏭 ' + esc(supplier) + '</h4>';
      items.forEach(function(o) {
        h += '<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid rgba(0,0,0,0.06)">';
        h += '<div><strong>' + esc(o.product_name) + '</strong><span style="color:var(--gray);font-size:13px;margin-left:8px">' + (o.quantity||1) + ' ' + esc(o.unit||'unité') + '</span>';
        h += '<div style="font-size:12px;color:var(--gray)">Commandé le ' + fmtD(o.ordered_at) + '</div></div>';
        h += '<button class="btn btn-success btn-sm" onclick="openReceiveModal(\'' + o.id + '\',\'' + esc(o.product_name).replace(/'/g,'\\\'') + '\')">✅ Réceptionner</button>';
        h += '</div>';
      });
      h += '</div>';
    });
...
function renderOrdersHistory() {
  var h = '';
  h += '<div class="card"><div class="card-header">📋 Commandes reçues</div><div class="card-body" id="orderHistoryContainer"><div style="text-align:center;padding:20px"><div class="loading"></div></div></div></div>';
...
async function loadAndRenderOrderHistory() {
  ...
  Object.keys(bySupplier).forEach(function(supplier) {
    ...
    html += '<h4 style="font-size:14px;font-weight:700;margin:0 0 10px;padding-bottom:6px;border-bottom:2px solid var(--success)">🏭 ' + esc(supplier) + ' <span class="badge badge-green">' + items.length + ' livraison' + (items.length > 1 ? 's' : '') + '</span></h4>';
    ...
    items.forEach(function(o) {
      ...
      html += '<div><strong>' + esc(o.product_name) + '</strong> — ' + (o.quantity||1) + ' ' + esc(o.unit||'unité');
      html += '<div style="font-size:12px;color:var(--gray)">Reçu le ' + fmtDT(o.received_at) + '</div></div>';
...
window.openReceiveModal = function(orderId, productName, photoOnly) {
  ...
  if (!photoOnly) {
    html += '<p style="margin-bottom:12px;font-size:14px">Confirmez la réception de <strong>' + productName + '</strong>.</p>';
  }
  html += '<div class="form-group"><label class="form-label">📸 Photo du BL ou de la facture <span style="font-size:12px;color:var(--gray)">(recommandé)</span></label>';
  ...
  html += '<div id="blPhotoPreview" style="display:none;text-align:center;padding:12px;border:2px solid var(--success);border-radius:8px;background:var(--success-bg)"><img id="blPhotoImg" class="photo-preview" style="max-width:300px"><br><button type="button" class="btn btn-ghost btn-sm" onclick="clearBLPhoto()">✕ Supprimer</button></div>';
  html += '</div>';
